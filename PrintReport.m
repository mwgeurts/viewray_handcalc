...
% PrintReport generates a report for the ViewRay Secondary Dose Calculation 
% Tool and saves it as a PDF file. The report contains a logo, loaded in
% the imread() statement below, tool user/version info, patient and machine
% info, and the calculation inputs and results for up to six beams.
%
% The report is generated by creating a figure, then populating the figure
% with text and figures, then saving the figure as a PDF using the saveas() 
% command, and finally closing the figure and opening the PDF using the 
% open() command.
%
